...
public class ChangeStepHandlerTests
{
     [Fact]
    public async Task
        HandleAsync_Throws_RecipeNotFoundException_When_There_Is_No_Recipe_With_Given_Id()
    {
        var command = new ChangeStep(Guid.NewGuid(),"StepToChange", "Step");
        ...
        HandleAsync_Calls_Repository_On_Success()
    {
        ...
        recipe.AddStep(new Domain.ValueObjects.Step("StepToChange"));
    ...
    private readonly ICommandHandler<ChangeStep> _commandHandler;
    public ChangeStepHandlerTests()
    {
        ...
        _commandHandler = new ChangeStepHandler(_repository, _userContext, _authorization);
    }
    #endregion
}