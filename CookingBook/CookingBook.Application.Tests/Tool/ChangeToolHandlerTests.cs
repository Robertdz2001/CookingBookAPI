...
public class ChangeToolHandlerTests
{
     [Fact]
    public async Task
        HandleAsync_Throws_RecipeNotFoundException_When_There_Is_No_Recipe_With_Given_Id()
    {
        var command = new ChangeTool(Guid.NewGuid(),"ToolToChange", "Tool",32);
        ...
        HandleAsync_Calls_Repository_On_Success()
    {
        ...
        recipe.AddTool(new Domain.ValueObjects.Tool("ToolToChange",32));
    ...
    private readonly ICommandHandler<ChangeTool> _commandHandler;
    public ChangeToolHandlerTests()
    {
        ...
        _commandHandler = new ChangeToolHandler(_repository, _userContext, _authorization);
    }
    #endregion
}