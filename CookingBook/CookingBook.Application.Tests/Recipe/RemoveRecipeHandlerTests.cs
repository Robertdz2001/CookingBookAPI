...
using CookingBook.Application.Commands.Handlers.Recipe;
...
public class RemoveRecipeHandlerTests
{
    ...
    [Fact]
    public async Task
        HandleAsync_Throws_RecipeNotFoundException_When_There_Is_No_Recipe_With_Given_Name()
    {
        var command = new RemoveRecipe(Guid.NewGuid());

        _repository.GetAsync(command.Id).Returns(default(Domain.Entities.Recipe));
    ...
    {
        var command = new RemoveRecipe(Guid.NewGuid());
        var recipe = new Domain.Entities.Recipe(Guid.NewGuid(), Guid.NewGuid(), "Recipe", "Url", 30, DateTime.UtcNow);

        _repository.GetAsync(command.Id).Returns(recipe);
        ...
        HandleAsync_Calls_Repository_On_Success()
    {
        ...
        await _repository.Received(1).DeleteAsync(Arg.Any<Domain.Entities.Recipe>());
    ...
    private readonly ICommandHandler<RemoveRecipe> _commandHandler;
    ...
    public RemoveRecipeHandlerTests()
    {
        ...
        _commandHandler = new RemoveRecipeHandler(_repository,_authorization,_userContext);
    }
