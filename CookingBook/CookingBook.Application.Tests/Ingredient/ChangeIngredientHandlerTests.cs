...
namespace CookingBook.Application.Tests.Ingredient;

public class ChangeIngredientHandlerTests
{
    [Fact]
    public async Task
        HandleAsync_Throws_RecipeNotFoundException_When_There_Is_No_Recipe_With_Given_Id()
    {
        var command = new ChangeIngredient(Guid.NewGuid(),"IngredientToChange", "Ingredient", 32, 32);
        ...
        HandleAsync_Calls_Repository_On_Success()
    {
        ...
        recipe.AddIngredient(new Domain.ValueObjects.Ingredient("IngredientToChange",32,32));
    ...
    private readonly ICommandHandler<ChangeIngredient> _commandHandler;
    public ChangeIngredientHandlerTests()
    {
        ...
        _commandHandler = new ChangeIngredientHandler(_repository, _userContext, _authorization);
    }
    #endregion
}