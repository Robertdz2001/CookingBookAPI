...
public class RemoveIngredientHandlerTests
{
    [Fact]
    public async Task
        HandleAsync_Throws_RecipeNotFoundException_When_There_Is_No_Recipe_With_Given_Id()
    {
        var command = new RemoveIngredient(Guid.NewGuid(), "Ingredient");
        ...
        HandleAsync_Calls_Repository_On_Success()
    {
        ...
        recipe.AddIngredient(new Domain.ValueObjects.Ingredient("Ingredient",32,32));
    ...
    private readonly ICommandHandler<RemoveIngredient> _commandHandler;
    public RemoveIngredientHandlerTests()
    {
        ...
        _commandHandler = new RemoveIngredientHandler(_repository, _userContext, _authorization);
    }
    #endregion
}