...
public class UserFactoryTests
{
    ...
    {
        var exception = Record.Exception(() => _factory.Create(Guid.Empty, "UserName", "PasswordHash","Url",1));
    ...
    {
        var exception = Record.Exception(() => _factory.Create(Guid.NewGuid(), "", "PasswordHash","Url",1));
    ...
    {
        var exception = Record.Exception(() => _factory.Create(Guid.NewGuid(), "UserName", "","Url",1));
    ...
    {
        User user = new User(Guid.NewGuid(), "UserName123", "PasswordHash123","Url");
        
        var exception = Record.Exception(() => user = _factory.Create(Guid.NewGuid(), "UserName", "PasswordHash","Url",1));
